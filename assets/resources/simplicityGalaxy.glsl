...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	float freqs[4];
	//Sound
	freqs[0] = sin(iGlobalTime);//texture2D( iChannel0, vec2( 0.01, 0.25 ) ).x;
	freqs[1] = .5;//texture2D( iChannel0, vec2( 0.07, 0.25 ) ).x;
	freqs[2] = 0.5;//texture2D( iChannel0, vec2( 0.15, 0.25 ) ).x;
	freqs[3] = .1;// texture2D( iChannel0, vec2( 0.30, 0.25 ) ).x;
